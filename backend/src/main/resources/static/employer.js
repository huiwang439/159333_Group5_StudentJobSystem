...
async function request(path, options = {}) {
  try {
    ...
    return {
      code: 0,
      message: "Network error. Please check whether the backend server is running.",
      data: null
    };
  }
}
...
function authHeaders(json = false) {
  const token = localStorage.getItem("employerToken"); // ⭐每次重新读取
  return {
...
async function saveEmployerProfile() {
  ...
  const profileResult = await request("/employers/profile", {
    method: "PUT",
  ...
  const businessLicenseUrl = document.getElementById("editBusinessLicenseUrl")?.value.trim() || "";
  const supportingDocumentUrl = document.getElementById("editSupportingDocumentUrl")?.value.trim() || "";

  if (businessLicenseUrl) {
    const verificationResult = await request(
      `/verification/submit?businessLicenseUrl=${encodeURIComponent(
        businessLicenseUrl
      )}&supportingDocumentUrl=${encodeURIComponent(supportingDocumentUrl)}`,
      {
        method: "POST",
        headers: authHeaders()
      }
...
async function loadJobs() {
  ...
  const result = await request("/jobs/my", {
    method: "GET",
    headers: authHeaders()
  });
...
async function updateJob() {
  ...
  const result = await request(`/jobs/${editingJobId}`, {
    method: "PUT",
    headers: authHeaders(true),
    body: JSON.stringify(payload)
  });
...
if (confirmDeleteBtn) {
  confirmDeleteBtn.onclick = async () => {
    ...
    if (deleteTargetId) {
      const result = await request(`/jobs/${deleteTargetId}`, {
        method: "DELETE",
        headers: authHeaders()
      });
    ...
    if (closeTargetId) {
      const result = await request(`/jobs/${closeTargetId}/close`, {
        method: "PATCH",
        headers: authHeaders()
      });
   ...
   if (actionTargetId && actionType) {
     const result = await request(`/applications/${actionTargetId}/status`, {
       method: "PATCH",
       headers: authHeaders(true),
...
async function loadApplications() {
  ...
  for (const job of targetJobs) {
    const result = await request(`/applications/job/${job.jobId}`, {
      method: "GET",
      headers: authHeaders()
    });
...
window.updateApplicationStatus = async function(id, status) {
  ...
  const result = await request(`/applications/${id}/status`, {
    method: "PATCH",
    headers: authHeaders(true),
...
function startApplicationAutoRefresh() {
  ...
  applicationRefreshTimer = setInterval(async () => {
    if (applicationsPage && applicationsPage.classList.contains("active") && employerToken) {
      await loadApplications();
    }

    if (dashboardPage && dashboardPage.classList.contains("active") && employerToken) {
...
if (navVerification) {
  navVerification.addEventListener("click", async () => {
    showPage("verification");
    if (employerToken) await loadEmployerProfile();
  });
}
...
if (navJobs) {
  navJobs.addEventListener("click", async () => {
    showPage("jobs");
    if (employerToken) await loadJobs();
  });
}
...
fillCreateFormDefault();
setCreateMode();
employerToken = localStorage.getItem("employerToken") || ""; // 先读

setLoginStatus(Boolean(employerToken)); // 再设置状态
if (employerToken) {
  loadEmployerProfile();
